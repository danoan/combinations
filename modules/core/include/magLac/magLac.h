...
namespace magLac{
    namespace Private
    {
        ...
        auto _initRange(unsigned int N,
                ...
                const std::initializer_list<TArgs>&... p_args)->Range< decltype( p_list.begin() ), decltype( p_args.begin() )... >
        {
    ...
    auto initRange(const std::initializer_list<TListType>& p_list,
                   const std::initializer_list<TArgs>&... p_args)->Core::Range< decltype( p_list.begin() ), decltype( p_args.begin() )... >
    {
    ...
    auto takeFromEach(const std::initializer_list<TListType>& p_list,
                      const std::initializer_list<TArgs>&... p_args)->Private::_CombinatorWrapper< decltype(Private::initRange<N>(p_list,p_args...)) >
    {
    ...
    auto initRange(const std::initializer_list<size_t>& NValues,
            ...
            const std::initializer_list<TArgs>&... p_args)->Core::Range< decltype( p_list.begin() ), decltype( p_args.begin() )... >
    {
        if (sizeof...(p_args)!= NValues.size()-1) throw std::runtime_error("NValues length is different than number of sets");
    ...
    auto takeFromEach(const std::initializer_list<size_t>& NValues,
            const std::initializer_list<TListType>& p_list,
            const std::initializer_list<TArgs>&... p_args)->Private::_CombinatorWrapper< decltype(Private::initRange(NValues,p_list,p_args...)) >
    {
